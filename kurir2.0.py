...
moving = False
paket_diambil = False
lokasi_pengambilan = (0, 0)
lokasi_pengantaran = (0, 0)
tahap = "ambil"
...
def draw():
    ...
    if tahap != "selesai":
        pygame.draw.rect(screen, (255, 255, 0), (lokasi_pengambilan[0] - flag_size//2, lokasi_pengambilan[1] + UI_HEIGHT - flag_size//2, flag_size, flag_size))
    pygame.draw.rect(screen, (255, 0, 0), (lokasi_pengantaran[0] - flag_size//2, lokasi_pengantaran[1] + UI_HEIGHT - flag_size//2, flag_size, flag_size))
...
lokasi_pengambilan = random_pos()
lokasi_pengantaran = random_pos()
...
while running:
    ...
    for event in pygame.event.get():
        if event.type == pygame.QUIT:
            ...
            if start_button.collidepoint(event.pos):
                tujuan = lokasi_pengambilan if tahap == "ambil" else lokasi_pengantaran
                path_result = astar(kurir, tujuan)
                if path_result:
                    path_index = 0
                    moving = True
    ...
    if moving and path_index < len(path_result):
        ...
        moving = False
        target = lokasi_pengambilan if tahap == "ambil" else lokasi_pengantaran
        if (kurir['x'], kurir['y']) == target:
            if tahap == "ambil":
                paket_diambil = True
                tahap = "antar"
                path_result = astar(kurir, lokasi_pengantaran)
                if path_result:
                    path_index = 0
                    moving = True
            elif tahap == "antar":
